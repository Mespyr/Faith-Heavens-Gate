...
Window::Window(std::ostream& error_log_ostream, const char* name,
...
uint32_t Window::center_x() { return WINDOW_CENTER_X; }

uint32_t Window::center_y() { return WINDOW_CENTER_Y; }

SDL_Texture* Window::load_texture(const char* file_path) {
    SDL_Texture* t = nullptr;
    t = IMG_LoadTexture(renderer, file_path);
    if (t == nullptr) log_error(error_log_ostream, "IMG_LoadTexture");
    return t;
}
...
void Window::render_texture(SDL_Texture* texture, SDL_Rect* frame,
                            SDL_Rect* position) {
    SDL_RenderCopyEx(renderer, texture, frame, position, 0, nullptr,
                     SDL_FLIP_NONE);
}
